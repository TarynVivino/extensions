Version 2.2.1 of Benchmarking Core (for Glulx only) by Dannii Willis begins here.

"A general purpose benchmarking test framework that produces statistically significant results."

"originally based on benchmark.js http://benchmarkjs.com"

The File of benchmarking dummy is called "dummy".
The benchmarking output file is an external file that varies.

Use benchmarking autotranscript translates as (- Constant BENCHMARKING_AUTOTRANSCRIPT; -)
...
A test case has a text called the description.

A test case has a text called the specification.
...
A test case has a text called author.

a test case has a text called the lambda.
...
To show the benchmark results:
	let num-fastest be 0;    
	...
	if the nonequivalent tests option is inactive:
		say "[paragraph break]The fastest test case[if num-fastest is 1] is[else]s are[end if] marked with a *. [if num-fastest > 1](They were statistically indistinguishable.[end if][run paragraph on]";
...
A last when play begins rule (this is the benchmark framework is taking over rule):
	if the benchmarking autotranscript option is active, try switching the story transcript on;
	else show the version info;
	say line break;    
	show the test case information;        
	say line break;    
