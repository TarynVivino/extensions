Version 1.1 of Trace Output by Taryn Michelle begins here.
...
To trace (T - text) with priority: do nothing;
To trace (T - text) as verbose: do nothing;
...
A trace-token is a kind of thing. A trace-token is usually traceable. A trace-token is always scenery. 
...
To trace (T - text) with priority, inline:
	if inline:
		trace T as priority_trace, inline;
	otherwise:
		trace T as priority_trace;

To trace (T - text) as verbose, inline:
	if inline:
		trace T as verbose_trace, inline;
	otherwise:
		trace T as verbose_trace;
